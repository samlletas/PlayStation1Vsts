...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=PsxReverb Demo Installer\n"
      else:
        line="OutputBaseFilename=PsxReverb Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the PsxReverb Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the PsxReverb installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=PsxReverb Demo installer\n"
     else:
       line="SetupWindowTitle=PsxReverb installer\n"
