...
def  main():
  ...
  installer = "\installer\PsxReverb Installer.exe"
   
  if demo:
    installer = "\installer\PsxReverb Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\PsxReverb manual.pdf" 
  ]
  ...
  ZIPNAME = "PsxReverb-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "PsxReverb-v" + FULLVERSIONSTR + "-win-demo.zip"
