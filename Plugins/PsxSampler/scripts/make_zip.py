...
def  main():
  ...
  installer = "\installer\PsxSampler Installer.exe"
   
  if demo:
    installer = "\installer\PsxSampler Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\PsxSampler manual.pdf" 
  ]
  ...
  ZIPNAME = "PsxSampler-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "PsxSampler-v" + FULLVERSIONSTR + "-win-demo.zip"
